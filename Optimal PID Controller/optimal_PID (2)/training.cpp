...
void train(void) {
    ...
    ofstream F("PID.txt", ios_base::trunc);
    ofstream Returns("Returns.txt", ios_base::trunc);

    for (episode = 1; episode < 200; episode++) {
        ...
        Returns << R << endl;
        F << policy->Parameters()[0] << ',' << policy->Parameters()[1] << ',' << policy->Parameters()[2] << endl;
        ...
        /*if ((episode > 25) && (lastEpisodes.size() == 15) && (sum > 599.00)) {
            break;
        }*/
